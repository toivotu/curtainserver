...
int main()
{
	...
	init_socket(&broker, "192.168.0.1", 1883, keepalive);

	// >>>>> CONNECT
    int status =  mqtt_connect(&broker);
	if ( status < 1) {
        printf("failed: %i", status);
        return -1;
    }
    ...
    mqtt_subscribe(&broker, "bitwise/409/right_curtain/position", &msg_id);
	// <<<<< SUBACK
	packet_length = read_packet(1);
	if(packet_length < 0)
	{
